...
# Save  factor scores
est_cfa_disc_sco = est_cfa2_disc_sco %>% select(ap_discusion,FOLIO) %>% as.data.frame()
```
...
## Guardar base de datos procesada
```{r}
est_cfa_sco <- left_join(x = est_data,y = est_cfa_sco,by ="FOLIO",suffix=c(" ",".y"))
est_cfa_sco = est_cfa_sco[,1:32]
est_data_sco <- left_join(x = est_cfa_sco,y = est_cfa_disc_sco,by ="FOLIO")
save(est_data_sco, file = "../input/data-proc/est_data_sco.rda")
```

# Base de datos conjunta
```{r}
data <- left_join(x = est_cfa_sco, y = apod_cfa_sco, by=c("FOLIO"="FOLIO_EST"), suffix=c(".est",".pad"))
