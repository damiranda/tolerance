...
biserial.cor(data_proc$igualdad.est,data_proc$Sexo, use = c("complete.obs"), level = 2)

corr.test(data_proc$igualdad.est, as.numeric(data_proc$Sexo))$p
```
```{r}
# Distribución igualitaria en el espacio público
biserial.cor(data_proc$esp_publico.est,data_proc$Sexo, use = c("complete.obs"), level = 2)

corr.test(data_proc$esp_publico.est, as.numeric(data_proc$Sexo))$p
```
```{r}
# Distribución igualitaria en el espacio privado
biserial.cor(data_proc$esp_privado.est,data_proc$Sexo, use = c("complete.obs"), level = 2)

corr.test(data_proc$esp_privado.est, as.numeric(data_proc$Sexo))$p
```
...
```{r, include = FALSE}
data_cor = data_proc
data_cor$Educacion <-as.numeric(data_proc$Educacion)
data_cor$Libros_apod_rec <-as.numeric(data_proc$Libros_apod_rec)
data_cor$quintiles_ingresos_pc_factor <- as.numeric(data_proc$quintiles_ingresos_pc_factor)
data_cor$quintiles_ingresos_pc_factor <- ifelse(data_cor$quintiles_ingresos_pc_factor==6,
                                                NA,
                                                data_cor$quintiles_ingresos_pc_factor)


corrplot.mixed(cor(dplyr::select(data_cor, igualdad.est, esp_publico.est, esp_privado.est, Educacion, Libros_apod_rec, quintiles_ingresos_pc_factor),
                   method="spearman",
                   use = "na.or.complete"))
```
...
cormat <- cor(dplyr::select(data_cor, igualdad.est, esp_publico.est, esp_privado.est, Educacion, Libros_apod_rec, quintiles_ingresos_pc_factor),
                   method="spearman",
                   use = "na.or.complete")
corrplot(
cormat,
...
```{r, include = FALSE}
corr.test(dplyr::select(data_cor, igualdad.est, esp_publico.est, esp_privado.est, Educacion, Libros_apod_rec, quintiles_ingresos_pc_factor),
                   method="spearman",
                   use = "complete")$p
```
...
```{r, include = FALSE}
corrplot.mixed(cor(dplyr::select(data_cor, igualdad.est, esp_publico.est, esp_privado.est, igualdad.pad, esp_publico.pad, esp_privado.pad),
                   use = "na.or.complete"))
```
```{r}
windowsFonts(A = windowsFont("Times New Roman"))
cormat <- cor(dplyr::select(data_cor, igualdad.est, esp_publico.est, esp_privado.est, igualdad.pad, esp_publico.pad, esp_privado.pad),
                   use = "na.or.complete")
...
```
```{r, include = FALSE}
corr.test(dplyr::select(data_cor, igualdad.est, esp_publico.est, esp_privado.est, igualdad.pad, esp_publico.pad, esp_privado.pad),
                   use = "complete")$p
```
...
```{r, include = FALSE}
corrplot.mixed(cor(dplyr::select(data_cor, igualdad.pad, esp_publico.pad, esp_privado.pad, Educacion, Libros_apod_rec, quintiles_ingresos_pc_factor),
                   method="spearman",
                   use = "na.or.complete"))
```
```{r}
windowsFonts(A = windowsFont("Times New Roman"))
cormat <- cor(dplyr::select(data_cor, igualdad.pad, esp_publico.pad, esp_privado.pad, Educacion, Libros_apod_rec, quintiles_ingresos_pc_factor),
                   method="spearman",
                   use = "na.or.complete")
...
```
```{r, include = FALSE}
corr.test(dplyr::select(data_cor, igualdad.pad, esp_publico.pad, esp_privado.pad, Educacion, Libros_apod_rec, quintiles_ingresos_pc_factor),
                   method="spearman",
                   use = "complete")$p
```
...
## Educacion
data_cor %>% dplyr::select(igualdad.est, Educacion, igualdad.pad) %>%
  pcor(method="spearman")
## Libros
data_cor %>% filter(!is.na(quintiles_ingresos_pc_factor)) %>%
  dplyr::select(igualdad.est, Libros_apod_rec, igualdad.pad) %>%
  pcor(method="spearman")
## Ingresos
data_cor %>% filter(!is.na(quintiles_ingresos_pc_factor)) %>%
  dplyr::select(igualdad.est, quintiles_ingresos_pc_factor, igualdad.pad) %>%
  pcor(method="spearman")
```
```{r}
# Espacio publico
## Educacion
data_cor %>% dplyr::select(esp_publico.est, Educacion,  esp_publico.pad) %>%
  pcor(method="spearman")
## Libros
data_cor %>% dplyr::select(esp_publico.est, Libros_apod_rec, esp_publico.pad) %>%
  pcor(method="spearman")
## Ingresos
data_cor %>% filter(!is.na(quintiles_ingresos_pc_factor)) %>%
  dplyr::select(esp_publico.est, quintiles_ingresos_pc_factor, esp_publico.pad) %>%
  pcor(method="spearman")
```
```{r}
# Espacio privado
##Educacion
data_cor %>% dplyr::select(esp_privado.est, Educacion, esp_privado.pad) %>%
  pcor(method="spearman")
## Libros
data_cor %>% dplyr::select(esp_privado.est, Libros_apod_rec, esp_privado.pad) %>%
  pcor(method="spearman")
## Ingresos
data_cor %>% filter(!is.na(quintiles_ingresos_pc_factor)) %>%
  dplyr::select(esp_privado.est, quintiles_ingresos_pc_factor, esp_privado.pad) %>%
  pcor(method="spearman")
```
...
```{r, include = FALSE}
corrplot.mixed(cor(dplyr::select(data_cor, igualdad.est, esp_publico.est, esp_privado.est, ap_discusion, mean_discusion, num_fem),
                   use = "na.or.complete"))
```
```{r}
windowsFonts(A = windowsFont("Times New Roman"))
cormat <- cor(dplyr::select(data_cor, igualdad.est, esp_publico.est, esp_privado.est, ap_discusion, mean_discusion, num_fem),
                   use = "na.or.complete")
...
```
```{r, include = FALSE}
corr.test(dplyr::select(data_cor, igualdad.est, esp_publico.est, esp_privado.est, ap_discusion, mean_discusion, num_fem),
                   use = "complete")$p
```
## Gráficos
```{r}
...
plot_ig_sexo <- ggplot(plotdata, 
  ...
  geom_hline(yintercept = mean_ig,
             color = "grey50",
             linetype = "solid") + 
 xlab("Promedio por sexo") + ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
          plot.title = element_text(family="Times New Roman", hjust=0.7, size=16, face="bold")) + ggtitle("Igualdad de derechos y oportunidades
...
plot_pub_sexo <- ggplot(plotdata, 
 ...
 xlab("Promedio por sexo") + ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
          plot.title = element_text(family="Times New Roman", hjust=0.5, size=16, face="bold")) + ggtitle("Distribución igualitaria de roles
...
plot_priv_sexo <- ggplot(plotdata, 
 ...
 xlab("Promedio por sexo") + ylab("Actitudes") + theme_classic()  +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
          plot.title = element_text(family="Times New Roman", hjust=0.5, size=16, face="bold")) +
 ylim(-0.3,0.3) + ggtitle("Distribución igualitaria de roles
...
```{r}
general <- cowplot::plot_grid(plot_ig_sexo,plot_pub_sexo,plot_priv_sexo, ncol=3)
ggsave(general, file = "../output/general_h1.png",device = "png",width = 30,height = 10,dpi = "retina",units = "cm")
```
![](../output/general_h1.png)
...
plot_ig_educ <- ggplot(plotdata, 
 ...
 xlab("Nivel de estudios del apoderado") + ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.title.x = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),) + ggtitle("                                                             ") + ylim(-0.3,0.3)
...
plot_ig_lib <- ggplot(plotdata, 
 ...
 xlab("Cantidad de libros en el hogar") + ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
          plot.title = element_text(family="Times New Roman", hjust=0.75, size=16, face="bold")) +
  ylim(-0.3,0.3) + ggtitle("Igualdad de derechos y oportunidades")
plot_ig_lib
...
plot_ig_ing <- ggplot(plotdata, 
 ...
 xlab("Ingresos per capita del hogar") + ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.title.x = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),) + ggtitle("                                                                         ") + 
ylim(-0.3,0.3)
plot_ig_ing
...
plot_pub_lib <- ggplot(plotdata, 
 ...
 xlab("Cantidad de libros en el hogar") + ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
          plot.title = element_text(family="Times New Roman", hjust=1, size=16, face="bold")) + ggtitle("Distribución igualitaria en la esfera pública") + ylim(-0.3,0.3)
plot_pub_lib
...
plot_priv_lib <- ggplot(plotdata, 
 ...
 xlab("Cantidad de libros en el hogar") + ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
          plot.title = element_text(family="Times New Roman", hjust=1, size=16, face="bold"))+ 
 ylim(-0.3,0.3) + ggtitle("Distribución igualitaria en la esfera privada")
plot_priv_lib
...
plot_priv_ing <- ggplot(plotdata, 
  ...
  geom_hline(yintercept = mean_ig,
             color = "grey50",
             linetype = "solid") + 
 xlab("Ingresos per capita del hogar") + ylab("Actitudes") + 
  theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.title.x = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
...
ggsave(plot = plot_grid(list(plot_ig_educ,plot_ig_lib,plot_ig_ing,plot_pub_educ,plot_pub_lib,plot_pub_ing,plot_priv_educ,plot_priv_lib,plot_priv_ing),
                        ...
                        tags = c(" ", " ", " "," ", " ", " ", " ", " ", " ")),
       filename = "../output/general_h2.png", width = 13, height = 8, dpi= "retina")
```
![](../output/general_h2.png)
...
plot_ig_act_apod <- ggplot(data_proc, aes(x=igualdad.pad, y=igualdad.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Actitudes del apoderado") + ylab("Actitudes del estudiante") + 
  theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.title.x = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          ...
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
          plot.title = element_text(family="Times New Roman", hjust=0.7, size=16, face="bold")) + ggtitle("                                                                         ") + 
ylim(-0.3,0.3)  + xlim(-1.5,0.5) + ggtitle("Igualdad de derechos y oportunidades
                                           ")
plot_ig_act_apod
ggsave(plot_ig_act_apod, file = "../output/ig_prop-act_apod.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")
```

```{r, include = FALSE}
#### Actitudes del estudiante hacia la distribución igualitaria de roles en la esfera pública

plot_pub_act_apod <- ggplot(data_proc, aes(x=esp_publico.pad, y=esp_publico.est)) +
  geom_point(color="#535151") + 
...
#pública, según las actitudes de su apoderado") + 
  theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
          plot.title = element_text(family="Times New Roman", hjust=0.5, size=16, face="bold")) + 
 ylim(-0.3,0.3)  + xlim(-1.5,0.5)+ ggtitle("Distribución igualitaria en 
la esfera publica")

plot_pub_act_apod
ggsave(plot_pub_act_apod, file = "../output/pub_prop-act_apod.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")
```

```{r, include = FALSE}
#### Actitudes del estudiante hacia la distribución igualitaria de roles en la esfera privada

plot_priv_act_apod <- ggplot(data_proc, aes(x=esp_privado.pad, y=esp_privado.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Actitudes del apoderado") + ylab("Actitudes del estudiante") + #ggtitle("Actitudes del estudiante hacia la distribución igualitaria de roles en la esfera 
...
plot_priv_act_apod
ggsave(plot_priv_act_apod, file = "../output/priv_prop-act_apod.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")
```

```{r}
general <- cowplot::plot_grid(plot_ig_act_apod,plot_pub_act_apod,plot_priv_act_apod, ncol=3)
ggsave(general, file = "../output/general_h3.png",device = "png",width = 30,height = 10,dpi = "retina",units = "cm")
```
![](../output/general_h3.png)
...
##### Proporción de niñas
plot_ig_prop_ninas <- ggplot(data_proc, aes(x=num_fem, y=igualdad.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Proporción de niñas") + #ggtitle("Igualdad de derechos y oportunidades,
#según la proporción de niñas en el aula") + 
  ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.title.x = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
...
ylim(-0.3,0.3)
plot_ig_prop_ninas
ggsave(plot_ig_prop_ninas, file = "../output/ig_prop-niñas.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")

##### Apertura a la discusión
plot_ig_ap_discusion <- ggplot(data_proc, aes(x=ap_discusion, y=igualdad.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Apertura a la discusión en el aula (individual)")+
  ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.title.x = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          ...
          plot.title = element_text(family="Times New Roman", hjust=0.75, size=16, face="bold")) +
  ylim(-0.3,0.3) + ggtitle("Igualdad de derechos y oportunidades")
plot_ig_ap_discusion
ggsave(plot_ig_ap_discusion, file = "../output/ig_ap_discusion_individual.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")

##### Percepción promedio sobre la apertura a la discusión en el aula
plot_ig_ap_discusion_prom <- ggplot(data_proc, aes(x=mean_discusion, y=igualdad.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Apertura a la discusión en el aula (general)") + #ggtitle("Actitudes del estudiante hacia la igualdad de derechos y oportunidades,
#según la percepción general del aula sobre la apertura a la discusión")  + 
  ylab("Actitudes") + theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          plot.title = element_text(family="Times New Roman", hjust=0.5, size=16)) + ggtitle("                                                                         ") + 
ylim(-0.3,0.3)
plot_ig_ap_discusion_prom
ggsave(plot_ig_ap_discusion_prom, file = "../output/ig_ap_discusion_promedio.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")
```
...
##### Proporción de niñas
plot_pub_prop_ninas <- ggplot(data_proc, aes(x=num_fem, y=esp_publico.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Proporción de niñas") + ylab("Actitudes") + #ggtitle("Actitudes del estudiante hacia la distribución igualitaria de roles en la esfera 
#pública, según la proporción de niñas en el aula") + 
 ...
 ylim(-0.3,0.3) + ggtitle("                                                                         ")
plot_pub_prop_ninas
ggsave(plot_pub_prop_ninas, file = "../output/priv_prop-niñas.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")

##### Apertura a la discusión
plot_pub_ap_discusion <- ggplot(data_proc, aes(x=ap_discusion, y=esp_publico.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Apertura a la discusión en el aula (individual)") + ylab("Actitudes") + #ggtitle("Actitudes del estudiante hacia la distribución igualitaria de roles en la esfera 
#pública, según su percepción sobre la apertura a la discusión en el aula") + 
  theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          axis.text.x = element_text(family="Times New Roman", colour = "black", size = 12, hjust = 0.5),
          plot.title = element_text(family="Times New Roman", hjust=1, size=16, face="bold")) + ggtitle("Distribución igualitaria en la esfera pública")+ 
 ylim(-0.3,0.3)
plot_pub_ap_discusion
ggsave(plot_pub_ap_discusion, file = "../output/pub_prop-ap_discusion_individual.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")

##### Percepción promedio sobre la apertura a la discusión en el aula

plot_pub_ap_discusion_prom <- ggplot(data_proc, aes(x=mean_discusion, y=esp_publico.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Apertura a la discusión en el aula (general)") + ylab("Actitudes") + #ggtitle("Actitudes del estudiante hacia la distribución igualitaria de roles en la esfera 
#pública, según la percepción general del aula sobre la apertura a la discusión") + 
  theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
         ...
         plot.title = element_text(family="Times New Roman", hjust=0.5, size=16)) + 
 ylim(-0.3,0.3) + ggtitle("                                                                         ")
plot_pub_ap_discusion_prom
ggsave(plot_pub_ap_discusion_prom, file = "../output/pub_prop-ap_discusion_promedio.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")
```
...
##### Proporción de niñas
plot_priv_prop_ninas <- ggplot(data_proc, aes(x=num_fem, y=esp_privado.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Proporción de niñas") + ylab("Actitudes") + #ggtitle("Actitudes del estudiante hacia la distribución igualitaria de roles en la esfera 
#privada, según la proporción de niñas en el aula") + 
  theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          plot.title = element_text(family="Times New Roman", hjust=0.5, size=16)) + 
 ylim(-0.3,0.3)+ ggtitle("                                                                         ")
plot_priv_prop_ninas
ggsave(plot_priv_prop_ninas, file = "../output/priv_prop-niñas.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")

##### Apertura a la discusión
plot_priv_ap_discusion <- ggplot(data_proc, aes(x=ap_discusion, y=esp_privado.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Apertura a la discusión en el aula (individual)") + ylab("Actitudes") + #ggtitle("Actitudes del estudiante hacia la distribución igualitaria de roles en la esfera 
#privada, según su percepción sobre la apertura a la discusión en el aula") + 
  theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          ...
          plot.title = element_text(family="Times New Roman", hjust=1, size=16, face="bold")) + ggtitle("Distribución igualitaria en la esfera pública")+ 
 ylim(-0.3,0.3)
plot_priv_ap_discusion
ggsave(plot_priv_ap_discusion, file = "../output/priv_prop-ap_discusion_individual.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")

##### Percepción promedio sobre la apertura a la discusión en el aula
plot_priv_ap_discusion_prom <- ggplot(data_proc, aes(x=mean_discusion, y=esp_privado.est)) +
  geom_point(color="#535151") + 
  geom_smooth(method=lm, color="#000000", fill="#D1CACA", se=TRUE) + xlab("Apertura a la discusión en el aula (general)") + ylab("Actitudes") + 
  theme_classic() +
    theme(axis.title.y = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.title.x = element_text(family="Times New Roman", size = 14, hjust = 0.5),
          axis.text.y = element_text(family="Times New Roman", colour = "black", size = 10, hjust = 0.5),
          ...
          plot.title = element_text(family="Times New Roman", hjust=0.5, size=16))+ 
 ylim(-0.3,0.3) + ggtitle("                                                                         ")
plot_priv_ap_discusion_prom
ggsave(plot_priv_ap_discusion_prom, file = "../output/priv_prop-ap_discusion_promedio.png",device = "png",width = 20,height = 13,dpi = "retina",units = "cm")
```

```{r}
ggsave(plot = plot_grid(list(plot_ig_prop_ninas,plot_ig_ap_discusion,plot_ig_ap_discusion_prom,plot_pub_prop_ninas,plot_pub_ap_discusion,plot_pub_ap_discusion_prom,plot_priv_prop_ninas,plot_priv_ap_discusion,plot_priv_ap_discusion_prom),
                        margin = c(0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01), 
                        tags = c(" ", " ", " "," ", " ", " ", " ", " ", " ")),
       filename = "../output/general_h5.png", width = 13, height = 8, dpi= "retina")
```
![](../output/general_h5.png)
...
```{r}
########################
# gráfico para congresos
########################

modelo_igualdad <- sjPlot::plot_model(reg_igualdad, type=c("std"), axis.labels = c("Conocimiento cívico","Región Metropolitana","Región del Maule (Ref. Antofagasta)", "Part. Pagado", "Part. Subvencionado (Ref. Municipal)","Proporción de mujeres en el aula","Apertura a la discusión (promedio aula)", "Apertura a la discusión (individual)", "Actitudes apoderado","No sabe/No responde [Quintil]", "Quintil de ingresos 5", "Quintil de ingresos 4", "Quintil de ingresos 3", "Quintil de ingresos 2 (Ref. Quintil 1)", "Más de 100 libros", "26-100 libros", "11-25 libros (Ref. 0-10 libros)", "Universidad o Posgrado","Educación Téc.Superior","Educación Media (Ref.8vo o menos)", "Sexo femenino (Ref. masculino)")) + ggtitle("Igualdad de derechos y oportunidades") + theme_grey() 

#modelo_igualdad <- sjPlot::plot_model(reg_igualdad, type=c("std")) + ggtitle("Igualdad de derechos y oportunidades") + theme_grey() 
modelo_igualdad
ggsave(modelo_igualdad, filename = "../output/modelo_igualdad.png",device = "png",width = 18,height = 12,dpi = "retina",units = "cm")
```
```{r}
########################
# gráfico para congresos
...
modelo_esp_publico <- sjPlot::plot_model(reg_esp.pub, type=c("std"), axis.labels = c(" "," "," ", " ", " "," "," ", " ", " "," ", " ", " ", " ", " ", " ", " ", " ", " "," "," ", " ")) + ggtitle("Distribución igualitaria en el espacio público") + theme_grey() 

modelo_esp_publico
ggsave(modelo_esp_publico, filename = "../output/modelo_esp_publico.png",device = "png",width = 18,height = 12,dpi = "retina",units = "cm")
```
```{r}
########################
# gráfico para congresos
...
modelo_esp_privado <- sjPlot::plot_model(reg_esp.priv, type=c("std"), axis.labels = c(" "," "," ", " ", " "," "," ", " ", " "," ", " ", " ", " ", " ", " ", " ", " ", " "," "," ", " ")) + ggtitle("Distribución igualitaria en el espacio privado") + theme_grey() 

modelo_esp_privado
ggsave(modelo_esp_privado, filename = "../output/modelo_esp_privado.png",device = "png",width = 18,height = 12,dpi = "retina",units = "cm")
```
```{r}
general <- cowplot::plot_grid(modelo_igualdad,modelo_esp_publico,modelo_esp_privado, ncol=3, rel_widths = c(1.35, 1, 1))
...
collapse_rows_dt2 <- data.frame(Variable = c(rep("Educación de los padres", 3), rep("Cantidad de libros", 3), rep("Quintiles de ingreso",5)),
                    ...
                    Porcentaje_Indirecto = c(
                                             paste0(round(med_out_iged1[["n.avg"]]*100,2),"%"), # entrega la proporción en %,
                    ...
                    Porcentaje_Indirecto = c(
                                             paste0(round(med_out_pubed1[["n.avg"]]*100,2),"%"),
                    ...
                    Porcentaje_Indirecto = c(
                                             paste0(round(med_out_prived1[["n.avg"]]*100,2),"%"),
...
kbl(collapse_rows_dt2, align = "c", col.names =  c("Variable","Categorías", "Efectos Directos", "Efectos Indirectos", "Efectos Totales","% de Efectos Indirectos", "Efectos Directos", "Efectos Indirectos", "Efectos Totales", "% de Efectos Indirectos", "Efectos Directos", "Efectos Indirectos", "Efectos Totales", "% de Efectos Indirectos")) %>%
  add_header_above(c(" " = 2,"Actitudes hacia  la igualdad de derechos y oportunidades"=4, "Actitudes hacia la distribución igualitaria de roles en la esfera pública"=4, "Actitudes hacia la distribución igualitaria de roles en la esfera privada"=4), bold = T)%>%  
  kable_paper("hover",full_width = F) %>%
     column_spec(1, bold = T) %>%
     collapse_rows(columns = 1:2, valign = "top")
...
```{r}
collapse_rows_dt3 <- data.frame(Variable = c(rep("Educación de los padres", 3), rep("Cantidad de libros", 3), rep("Quintiles de ingreso",5)),
                    Categorias = c(sjlabelled::get_labels(data_proc$Educacion)[-1],
                                   sjlabelled::get_labels(data_proc$Libros_apod_rec)[-1],
                                   sjlabelled::get_labels(data_proc$quintiles_ingresos_pc_factor)[-1]),
                    ...
                    Porcentaje_Indirecto = c(
                                             paste0(round(med_out_iged1.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iged2.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iged3.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iglib1.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iglib2.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iglib3.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iging1.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iging2.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iging3.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iging4.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_iging5.2[["n.avg"]]*100,2),"%")
                                             ),
                    ...
                    Efectos_Totales = c(paste0(round(med_out_pubed1.2[["tau.coef"]],3), gtools::stars.pval(med_out_pubed1.2[["tau.p"]])),
                     ...
                     Porcentaje_Indirecto = c(
                                             paste0(round(med_out_pubed1.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_pubed2.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_pubed3.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_publib1.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_publib2.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_publib3.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_pubing1.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_pubing2.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_pubing3.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_pubing4.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_pubing5.2[["n.avg"]]*100,2),"%")
                                             ),
                    ...
                    Efectos_Totales = c(paste0(round(med_out_prived1.2[["tau.coef"]],3), gtools::stars.pval(med_out_prived1.2[["tau.p"]])),
                     ...
                     Porcentaje_Indirecto = c(
                                             paste0(round(med_out_prived1.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_prived2.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_prived3.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_privlib1.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_privlib2.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_privlib3.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_priving1.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_priving2.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_priving3.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_priving4.2[["n.avg"]]*100,2),"%"),
                                             paste0(round(med_out_priving5.2[["n.avg"]]*100,2),"%")
                                             )
                    )
                    
kbl(collapse_rows_dt3, align = "c", col.names =  c("Variable","Categorías", "Efectos Directos", "Efectos Indirectos", "Efectos Totales","% de Efectos Indirectos", "Efectos Directos", "Efectos Indirectos", "Efectos Totales", "% de Efectos Indirectos", "Efectos Directos", "Efectos Indirectos", "Efectos Totales", "% de Efectos Indirectos")) %>%
  add_header_above(c(" " = 2,"Actitudes hacia  la igualdad de derechos y oportunidades"=4, "Actitudes hacia la distribución igualitaria de roles en la esfera pública"=4, "Actitudes hacia la distribución igualitaria de roles en la esfera privada"=4), bold = T)%>%  
  kable_paper("hover",full_width = F) %>%
     column_spec(1, bold = T) %>%
     collapse_rows(columns = 1:2, valign = "top")
