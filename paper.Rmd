...
[Rol de la escuela: transmisión normativa, orientación explícita curricular, escuela como contexto]
[Este estudio: combina diferentes aspectos de la familia y la escuela como agentes de socialización de actitudes hacia la igualdad de género en diversos ámbitos (público-privado). Destacar aquello que puede ser considerado novedoso - avance en conocimiento - por ejemplo, evaluar dos agentes relevantes de manera simultánea, combina dos modelos explicativos (recursos y prácticas de socialización), etc.]
[Objetivos]
...
data_proc$igualdad.est <- set_label(x = data_proc$igualdad.est,label = "Students attitudes towards equal rights and opportunities between men and women")
data_proc$esp_publico.est <- set_label(x = data_proc$esp_publico.est,label = "Students attitudes towards equal distribution of roles in the public sphere")
data_proc$esp_privado.est <- set_label(x = data_proc$esp_privado.est,label = "Students attitudes towards equal distribution of roles in the private sphere")
data_proc$igualdad.pad <- set_label(x = data_proc$igualdad.pad,label = "Parents attitudes towards equal rights and opportunities between men and women")
data_proc$esp_publico.pad <- set_label(x = data_proc$esp_publico.pad,label = "Parents attitudes towards equal distribution of roles in the public sphere")
data_proc$esp_privado.pad <- set_label(x = data_proc$esp_privado.pad,label = "Parents attitudes towards equal distribution of roles in the private sphere")
data_proc$Sexo <- set_label(x = data_proc$Sexo,label = "Students sex")
data_proc$Educacion <- set_label(x = data_proc$Educacion,label = "Highest parental educational level") # Etiquetar variable
data_proc$Libros_apod_rec <- set_label(x = data_proc$Libros_apod_rec,label = "Number of books in home")
data_proc$quintiles_ingresos_pc_factor <- set_label(x = data_proc$quintiles_ingresos_pc_factor, label = "Per capita household income in quintiles")
data_proc$ap_discusion <- set_label(x = data_proc$ap_discusion,label = "Students' perception of openness in classroom discussions")
data_proc$mean_discusion <- set_label(x = data_proc$mean_discusion,label = "Course average perception of openness in classroom discussions")
data_proc$num_fem <- set_label(x = data_proc$num_fem,label = "Proportion of girls in the classroom")
data_proc$c_civic <- set_label(x = data_proc$c_civic, label = "Civic knowledge")
...
descr$Sexo <- factor(descr$Sexo,
                     levels = c("Niño", "Niña"),
                     labels = c("Boy", "Girl"))

descr$Educacion <- factor(descr$Educacion, 
...
descr$Libros_apod_rec <- factor(descr$Libros_apod_rec, 
...
descr$quintiles_ingresos_pc_factor <- factor(descr$quintiles_ingresos_pc_factor, 
                                                 levels = c("Quintil 1",
                                                            "Quintil 2",
                                                            "Quintil 3",
                                                            "Quintil 4",
                                                 ...
                                                 labels = c("Quintile 1",
                                                            ...
                                                            "Quintile 5",
                                                            "Missing"))
descr$dependencia <- factor(descr$dependencia,
                            levels = c("Municipal",
                                       "Part. Subvencionado",
                                       "Part. Pagado"),
                            labels = c("Public",
                                       "Subsidized private",
                                       "Private"))
descr$REGION <- factor(descr$REGION,
...
descr$Sexo <- set_label(x = descr$Sexo,label = "Student sex")
descr$Educacion <- set_label(x = descr$Educacion,label = "Highest parental educational level") # Etiquetar variable
descr$Libros_apod_rec <- set_label(x = descr$Libros_apod_rec,label = "Number of books in home")
descr$quintiles_ingresos_pc_factor <- set_label(x = descr$quintiles_ingresos_pc_factor, label = "Per capita household income in quintiles")
descr$dependencia <- set_label(x=descr$dependencia, label = "Administrative dependency of school")
descr$REGION <- set_label(x=descr$REGION, label = "Region")
...
kable(tab,
      ...
      booktabs = T, 
      linesep = "",
      col.names =  c("Independent variables","Equal rights and opportunities","Equal distribution of roles in public sphere", "Equal distribution of roles in private sphere"),
      caption = "Main correlations.",
      align=c('l','c','c','c')) %>%
  kable_styling(
    full_width = T,
    latex_options = c("hold_position"),
  ...
  footnote("***p < 0.001; **p < 0.01; *p < 0.05", footnote_as_chunk=T, general_title="Note:") %>%
  add_header_above(c(" ", "Dependent variables: Student attitudes towards gender equality" = 3)) #%>%
  #collapse_rows(columns = 1:2,valign = "middle")
```
...
footnote<- "\\item %stars. \\item All models are controlled for region of residence (p>0.05), administrative dependency of the school (p>0.05) and level of civic knowledge of the student ($p$<0.05)"
scale_box <- 0.825
```

### Actitudes del estudiante hacia la igualdad de derechos y oportunidades entre hombres y mujeres
En la Tabla 5 se presentan los principales resultados obtenidos a partir del análisis multinivel realizado para la dimensión de actitudes de los estudiantes hacia la igualdad de derechos y oportunidades entre hombres y mujeres. Para esta dimensión la correlación intraclase obtenida representa que la variación entre escuelas corresponde a un 23% de la varianza de las actitudes de los estudiantes.
...
knitreg(list(reg_igualdad1,reg_igualdad2,reg_igualdad3,reg_igualdad4,
        ...
        custom.coef.names = c(coef_names,
          ...
          "Openness in classroom discussions (individual)",
          "Openness in classroom discussions (group)",
...
En la Tabla 6 se presentan los resultados obtenidos de las actitudes de los estudiantes hacia la dimensión de distribución igualitaria de roles en la esfera pública. Para esta dimensión la correlación intraclase obtenida representa que la variación entre escuelas corresponde a un 26% de la varianza de las actitudes de los estudiantes.
...
```{r echo=FALSE, fig.cap="Students' attitudes towards gender equality according to the student's sex", fig.align = 'center', out.width = '97%',out.height='97%'}
...
```{r echo=FALSE, fig.cap="Students' attitudes towards gender equality according to the socioeconomic resources of the family.", fig.align = 'center', out.width = '97%',out.height='97%'}
...
```{r echo=FALSE, fig.cap="Students' attitudes towards gender equality according to their parent's attitudes.", fig.align = 'center', out.width = '97%',out.height='97%'}
...
```{r echo=FALSE, fig.cap="Student attitudes towards gender equality according to school characteristics.", fig.align = 'center', out.width = '97%',out.height='97%'}
...
```{r, echo=FALSE, results='asis'}
caption <- "Multilevel Regression Model for Parent Attitudes"
knitreg(list(h4_igualdad.apod,h4_esp_pub.apod,h4_esp_priv.apod), 
        custom.model.names = c(
          "Equal Rights and Opportunities",
          "Equal Distribution in public sphere",
          "Equal Distribution in private sphere"
        ), 
